...
public class SQL {
     ...
     * @param sql query for prepared statement
     * 
     * @param artist_name artist name to search by
     */
    public static void ps_AddArtist(String sql, int artist_ID, String artist_name, String DOB) {
    	try {
            ps = GRS.conn.prepareStatement(sql);
            ...
            System.out.println(rowsAffected + " artist(s) added!");

            // Show the inserted artist
            String selectSql = "SELECT * FROM ARTIST WHERE Artist_ID = ?";
            ps = GRS.conn.prepareStatement(selectSql);
            ps.setInt(1, artist_ID);
            sqlQuery(GRS.conn, ps);
        } catch (SQLException e) {
